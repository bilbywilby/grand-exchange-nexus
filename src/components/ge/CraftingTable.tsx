...
import { getCraftingProfits } from '@/lib/api';
...
export function CraftingTable({ batchQuantity, onItemClick }: { batchQuantity: number, onItemClick: (id: number) => void }) {
    ...
    const handleRefresh = () => queryClient.invalidateQueries({ queryKey: ['crafting'] });
    const handleExportCSV = () => {
        ...
        link.download = 'ge_crafting_profits.csv';
        link.click();
    ...
    return (
        <div className="max-w-7xl mx-auto w-full">
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <h1 className="text-4xl font-bold tracking-tight text-yellow-400">Crafting Profits</h1>
                <p className="mt-2 text-lg text-slate-300">Find profitable items to craft, from jewellery to orbs.</p>
            </motion.div>
            <Alert className="mt-8 bg-blue-900/20 border-blue-500 text-blue-300">
                ...
                <AlertDescription>Profits are based on buying raw materials and selling the finished products. A glassblowing pipe or appropriate moulds are required.</AlertDescription>
            </Alert>
            <Card className="mt-8 bg-slate-900/70 border-slate-800 backdrop-blur-sm">
                <CardHeader>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div><CardTitle>Profit/Loss per Item</CardTitle><CardDescription>Last updated: {lastUpdated.toLocaleTimeString()}</CardDescription></div>
                    ...
                    <div className="overflow-x-auto">
                        <Table>
                            <TableHeader><TableRow className="hover:bg-transparent border-slate-800">
                                <TableHead onClick={() => handleSort('name')} className="cursor-pointer hover:text-white text-yellow-400"><div className="flex items-center">Item {sort.key === 'name' && <ArrowUpDown className="ml-2 h-4 w-4" />}</div></TableHead>
                                <SortableHeader sortKey="profitPer"><span className="text-yellow-400">Profit/Item</span></SortableHeader>
