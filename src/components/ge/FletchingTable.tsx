...
import { getFletchingProfits } from '@/lib/api';
...
export function FletchingTable({ batchQuantity, onItemClick }: { batchQuantity: number, onItemClick: (id: number) => void }) {
    ...
    const handleRefresh = () => queryClient.invalidateQueries({ queryKey: ['fletching'] });
    const handleExportCSV = () => {
        ...
        link.download = 'ge_fletching_profits.csv';
        link.click();
    ...
    return (
        <div className="max-w-7xl mx-auto w-full">
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <h1 className="text-4xl font-bold tracking-tight text-yellow-400">Fletching Profits</h1>
                <p className="mt-2 text-lg text-slate-300">Discover profitable items to fletch for great XP and GP.</p>
            </motion.div>
            <Alert className="mt-8 bg-blue-900/20 border-blue-500 text-blue-300">
                ...
                <AlertDescription>A knife is required for fletching logs. Profits assume you are buying logs and selling the unstrung bows.</AlertDescription>
            </Alert>
            <Card className="mt-8 bg-slate-900/70 border-slate-800 backdrop-blur-sm">
                <CardHeader>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div><CardTitle>Profit/Loss per Action</CardTitle><CardDescription>Last updated: {lastUpdated.toLocaleTimeString()}</CardDescription></div>
