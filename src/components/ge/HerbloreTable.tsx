...
type SortKey = 'name' | 'profit_per_potion' | 'cost_per_potion' | 'sell_price_unf';
const containerVariants = { hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.02 } } };
const itemVariants = { hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } };
export function HerbloreTable({ batchQuantity, onItemClick }: { batchQuantity: number, onItemClick: (id: number) => void }) {
    const [searchTerm, setSearchTerm] = useState('');
    const [lastUpdated, setLastUpdated] = useState(new Date());
    const [sort, setSort] = useState<{ key: SortKey; dir: 'asc' | 'desc' }>({ key: 'profit_per_potion', dir: 'desc' });
    ...
    const totals = useMemo(() => {
        if (!sortedData) return { totalCost: 0, totalSell: 0, totalProfit: 0 };
    ...
    const handleExportCSV = () => {
        ...
        const rows = sortedData.map(item => [`"${item.name.replace(/"/g, '""')}"`, item.profit_per_potion, item.cost_per_potion, item.sell_price_unf].join(','));
        const totalsRow = [`"Totals for ${batchQuantity} potions"`, totals.totalProfit, totals.totalCost, totals.totalSell].join(',');
        const csv = [headers.join(','), ...rows, '', 'Batch Totals', totalsRow].join('\n');
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = 'ge_herblore_profits.csv';
        link.click();
    ...
    return (
        <div className="max-w-7xl mx-auto w-full">
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                ...
                <p className="mt-2 text-lg text-slate-300">Calculate profits from making unfinished potions.</p>
            </motion.div>
            <Alert className="mt-8 bg-blue-900/20 border-blue-500 text-blue-300">
                ...
                <AlertDescription>Requires Desert Hard Diary or Herblore Cape for Zahur to clean herbs for 200gp each. You clean the first herb yourself.</AlertDescription>
            </Alert>
            <Card className="mt-8 bg-slate-900/70 border-slate-800 backdrop-blur-sm">
                <CardHeader>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div><CardTitle>Profit/Loss per Herb</CardTitle><CardDescription>Last updated: {lastUpdated.toLocaleTimeString()}</CardDescription></div>
                    ...
                    <div className="overflow-x-auto">
                        <Table>
                            <TableHeader><TableRow className="hover:bg-transparent border-slate-800">
                                <TableHead onClick={() => handleSort('name')} className="cursor-pointer hover:text-white text-yellow-400"><div className="flex items-center">Herb {sort.key === 'name' && <ArrowUpDown className="ml-2 h-4 w-4" />}</div></TableHead>
                                <SortableHeader sortKey="profit_per_potion"><span className="text-yellow-400">Profit/Potion</span></SortableHeader>
                                <SortableHeader sortKey="cost_per_potion"><span className="text-red-400">Cost/Potion</span></SortableHeader>
                                <SortableHeader sortKey="sell_price_unf"><span className="text-green-400">Sell Price (UNF)</span></SortableHeader>
                            </TableRow></TableHeader>
                            {isLoading ? (
                                <TableBody>{Array.from({ length: 10 }).map((_, i) => (<TableRow key={i} className="border-slate-800"><TableCell colSpan={4}><Skeleton className="h-5 w-full" /></TableCell></TableRow>))}</TableBody>
                            ) : error ? (
                                ...
                                <motion.tbody variants={containerVariants} initial="hidden" animate="visible">
                                    {sortedData.map((item) => (
                                        <motion.tr key={item.name} variants={itemVariants} className="border-slate-800 hover:bg-slate-800/50 cursor-pointer" onClick={() => onItemClick(item.unf_id)}>
            ...
            <Card className="mt-6 bg-slate-900/70 border-slate-800 backdrop-blur-sm">
                <CardHeader><CardTitle>Batch Totals (for {batchQuantity.toLocaleString()} potions)</CardTitle></CardHeader>
